...
router.post(
    '/like/:id', passport.authenticate('jwt', {
        session: false
    }),
    (req , res) => {
        Profile.findOne({
...
router.post(
    '/unlike/:id', passport.authenticate('jwt', {
        session:false
    }),
    (req , res) => {
        Profile.findOne({
...
router.post(
    '/comment/:id', passport.authenticate('jwt', {
        session: false
    }),
    (req, res) => {
        const {errors, isValid} = validatePostInput(req.body)
        ...
        .then(post => {
            ...
            // Add new comments to comments array
        ...
        Post.findById(req.params.id)
            .then(post => {
                if(post.comments.filter(
                    ...
                    post.save().than(post => res.json(post))
            })
            .catch(err => {
                res.status(404).json({
                    postnotfound: 'No post found'
